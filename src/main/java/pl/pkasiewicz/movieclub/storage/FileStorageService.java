package pl.pkasiewicz.movieclub.storage;
...
class FileStorageService {
    ...
    private String saveFile(MultipartFile file, String storageLocation) {
        Path filePatch = createFilePatch(file, storageLocation);
        try {
            Files.copy(file.getInputStream(), filePatch, StandardCopyOption.REPLACE_EXISTING);
            return filePatch.getFileName().toString();
        } catch (IOException e) {
            throw new UncheckedIOException(e);
        }
    }

    private Path createFilePatch(MultipartFile file, String storageLocation) {
